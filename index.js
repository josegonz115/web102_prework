...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i += 1){
        ...
        gameCard.innerHTML = 
        `<img src="${games[i].img}" class="game-img" alt="${games[i].name} image" />
        <h3 class="game-header">${games[i].name}</h3>
...
const unfundedGamesCount = GAMES_JSON.reduce( (total, game) => {
    return game.pledged < game.goal ? total + 1 : total; }, 0);

// create a string that explains the number of unfunded games using the ternary operator
const allGamesStr = totalGames===1 ? "game": "games";
const unfundedAmtStr = unfundedGamesCount===1 ? "game": "games";
const unfundedGamesString = 
`A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} ${allGamesStr}. 
However, ${unfundedGamesCount} ${unfundedAmtStr} still have not yet met their goal.`;
...
/************************************************************************************
 * Adding a search feature for the games
...
function searchResults() {
    ...
    let filteredGames = GAMES_JSON.filter( (game) => {return game.name.toLowerCase().includes(searchTerm) });
    deleteChildElements(gamesContainer);
